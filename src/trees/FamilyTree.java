...
public class FamilyTree {
	...
	private static class TreeNode {
		...
		ArrayList<TreeNode> collectAncestorsToList() {
			...
			TreeNode parentToAdd = this.parent;

			while (parentToAdd != null) {

				ancestors.add(this.parent);
				parentToAdd = parentToAdd.parent;
			}
			ancestors.add(this.parent);
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Missing colon"); //throw a TreeException with a useful message
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("Node with name '" + name1 + "' not found");  //Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("Node with name '" + name2 + "' not found"); //Throw TreeException with a useful message
